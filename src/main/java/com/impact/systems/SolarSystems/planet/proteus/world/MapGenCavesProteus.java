...
public class MapGenCavesProteus extends MapGenBaseMeta {
    public static final int BREAK_THROUGH_CHANCE = 25;

    public MapGenCavesProteus() {
    }
    ...
    protected void generateCaveNode(long par1, int par3, int par4, Block[] blockIdArray, byte[] metaArray, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16, double par17) {
        ...
        for (boolean flag1 = random.nextInt(6) == 0; par15 < par16; ++par15) {
            ...
            if (!flag && par15 == k1 && par12 > 1.0F && par16 > 0) {
                this.generateCaveNode(random.nextLong(), par3, par4, blockIdArray, metaArray, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 - 1.5707964F, par14 / 3.0F, par15, par16, 1.0D);
                this.generateCaveNode(random.nextLong(), par3, par4, blockIdArray, metaArray, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 + 1.5707964F, par14 / 3.0F, par15, par16, 1.0D);
                return;
            }
                    ...
                    for (int j3 = l1; j3 < i2; ++j3) {
                        for (localY = l2; localY < i3; ++localY) {
                            for (int i4 = k2 + 1; i4 >= j2 - 1; --i4) {
                                if (i4 < 128 && i4 != j2 - 1 && j3 != l1 && j3 != i2 - 1 && localY != l2 && localY != i3 - 1) {
                                    i4 = j2;
                                }
                            }
                        }
                            ...
                            for (int localZ = l2; localZ < i3; ++localZ) {
                                ...
                                if (xfactorSq + zfactorSq < 1.0D) {
                                    ...
                                    if (yfactor > -0.7D && xfactorSq + yfactorSq + zfactorSq < 1.0D) {
                                        if (blockIdArray[coords] == IGM_Blocks.ProteusBlocks && metaArray[coords] == 0 && random.nextInt(25) == 0) {
                                            blockIdArray[coords] = Blocks.air;
                                        } else if (blockIdArray[coords] == IGM_Blocks.ProteusBlocks && metaArray[coords] == 1) {
                                            blockIdArray[coords] = Blocks.air;
                                        } else if (blockIdArray[coords] == IGM_Blocks.ProteusBlocks && metaArray[coords] == 2) {
                                            blockIdArray[coords] = Blocks.air;
                                        }
                                    }
                                }
